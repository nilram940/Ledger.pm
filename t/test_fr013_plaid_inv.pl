#!/usr/bin/perl
# FR-013: Plaid investment account parser test.
# Verifies addinvestments(): commodity (ticker symbol), share quantity,
# cost basis, and per-security account name derivation.
...
copy('fr013_plaid_inv.json', "$dir/Plaid-2026-02.json") or die "copy plaid inv: $!";

my $ledger = Ledger->new(file => $ldg);

print "=== Importing Plaid-2026-02.json (investment) ===\n";
$ledger->fromStmt("$dir/Plaid-2026-02.json", {}, {});

my $expected_account = 'Assets:Brokerage:My Brokerage:AAPL';
...
sub check {
    ...
    my $bal_present = ($content =~ /= \$5900\.00/);

    print "\n=== FR-013 Plaid Investment RESULT ===\n";
    ...
    printf "date = 2026/02/05:                    %s  (want yes, got %s)\n",
        ...
        $buy_date;
    printf "account = correct:                    %s\n  (want: %s\n   got:  %s)\n",
        ($buy_acct eq $want_account) ? 'yes' : 'NO',
        $want_account, $buy_acct;
    printf "Balance assertion \$5900.00 in file:   %s  (want yes)\n",
        $bal_present ? 'yes' : 'NO';
